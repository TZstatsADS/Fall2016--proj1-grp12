...
ACS_pus_1 = NACS_pus[ ,c("SERIALNO","MAR","MARHD","MARHD","MARHM","MARHT","MARHW","MARHYP","OCCP"
...
## logistic regression 
class_model = glm(MAR_New ~ Occupation+Length.of.Work+PINCP+SCHL_code+AGEP+Race, data = na.omit(ACS_pus_2),family = "binomial")
...
class_model2 = glm(MAR_New ~ Length.of.Work+PINCP+SCHL_code+AGEP+Race, data = na.omit(ACS_pus_2),family = "binomial")
...
ggplot(ACS_pus_2, aes(Occupation,fill = Marriage.Status)) + 
  geom_bar(position = "fill") +
  xlab("Occupation") +
  ylab("Percentage of Marriage Status") + 
  scale_fill_manual("Marriage Status",values = alpha( c("firebrick", "dodgerblue4"), 1))  

ggplot(ACS_pus_2, aes(Length.of.Work,fill = Marriage.Status)) + 
  geom_bar(position = "fill")+
  xlab("Length of Work") +
  ylab("Percentage of Marriage Status") + 
  scale_fill_manual("Marriage Status",values = alpha( c("firebrick", "dodgerblue4"), 1))  

ggplot(ACS_pus_2, aes(SCHL_code,fill = Marriage.Status)) + 
  geom_bar(position = "fill")+
  xlab("Education") +
  ylab("Percentage of Marriage Status") + 
  scale_fill_manual("Marriage Status",values = alpha( c("firebrick", "dodgerblue4"), 1))  

ggplot(ACS_pus_2, aes(Race,fill = Marriage.Status)) + 
  geom_bar(position = "fill")+
  xlab("Race") +
  ylab("Percentage of Marriage Status") + 
  scale_fill_manual("Marriage Status",values = alpha( c("firebrick", "dodgerblue4"), 1))  
...
ggplot(ACS_pus_2, aes(PINCP,fill = Marriage.Status)) + 
...
# Occupency
ggplot(na.omit(ACS_pus_2), aes(Occupation, ..count..)) + 
  geom_bar(aes(fill = ..count..))+
  xlab("Occupation")
# Length of work
ggplot(na.omit(ACS_pus_2), aes(Length.of.Work, ..count..)) + 
  geom_bar(aes(fill = ..count..))+
  xlab("Length of Work")
# Education
ggplot(ACS_pus_2, aes(SCHL_code, ..count..)) + 
  geom_bar(aes(fill = ..count..))+
  xlab("Education")
# Age
ggplot(ACS_pus_2, aes(AGEP, ..count..)) + 
  geom_bar(aes(fill = ..count..))+
  xlab("Age")
#Race
ggplot(ACS_pus_2, aes(Race, ..count..)) + 
  ...
  xlab("Race")
# Personal Income
ggplot((ACS_pus_2), aes(PINCP, ..count..))+
...
################################ Reference #############################
## Test of variabe FES 
ACS_pus_1 = NACS_pus[ ,"FES"]
head(NACS_pus$FES)               


## used variables
"SERIALNO","MAR","MARHD","MARHD","MARHM","MARHT","MARHW","MARHYP","OCCP",
"WKW","POWSP","PINCP","FINCP","SCHL","FOD1P","AGEP","RAC1P","RAC2P","POBP",
"FSERIALNOP","FMARP","FFESP","FHHTP","FSSMCP","FMARHDP","FMARHMP","FMARHTP",
"FMARHWP","FMARHYPP","FMSPP","FESRP","FOCCPP","FWKWP","FPOWSPP","FPINCPP",
"FFINCPP","FSCHLP","FFOD1PP","FAGEPP","FRAC1PP","FRAC2PP","FPOBPP"
## out of senariro
"FES"
## unused variables
,"SSMC","HHT","MSP","ESR"